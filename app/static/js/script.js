...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // Initialize education field logic
...
function initializeEducationField() {
  ...
  educationField.addEventListener('change', function () {
      ...
      // Update progress bar after changes
      updateProgressBar();
  });
}
...
function validateForm() {
  ...
  const educationField = document.getElementById('education');
  if (educationField) {
    const educationValue = educationField.value;
    
    // Check undergraduate sublevel if undergraduate is selected
    if (educationValue === 'undergraduate') {
      const sublevelSelect = document.getElementById('undergradSublevel');
      if (sublevelSelect && !sublevelSelect.value.trim()) {
        setFieldError(sublevelSelect, 'This field is required');
        isValid = false;
      }
    }
    
    // Check pre-tertiary age if pre-tertiary is selected
    if (educationValue === 'pre-tertiary') {
      const ageInput = document.getElementById('preTertiaryAge');
      if (ageInput && !ageInput.value.trim()) {
        setFieldError(ageInput, 'This field is required');
        isValid = false;
      }
    }
  }
...
function updateProgressBar() {
  ...
  // Add dynamic fields based on education selection
  const educationField = document.getElementById('education');
  if (educationField) {
    const educationValue = educationField.value;
    if (educationValue === 'undergraduate') {
      fields.push('undergradSublevel');
    } else if (educationValue === 'pre-tertiary') {
      fields.push('preTertiaryAge');
    }
  }
